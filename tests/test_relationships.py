...
async def test_relationship_creation_and_rejection(client, auth_headers):
    ...
    resp_a = await client.post("/api/v1/assets", json=asset_a_payload, headers=auth_headers)
    resp_b = await client.post("/api/v1/assets", json=asset_b_payload, headers=auth_headers)
    ...
    response = await client.post("/api/v1/relationships", json=rel_payload, headers=auth_headers)
    ...
    response = await client.post("/api/v1/relationships", json=bad_payload_source, headers=auth_headers)
    assert response.status_code == 404
    assert response.json()["detail"]["error"] == "not_found"

    response = await client.post("/api/v1/relationships", json=bad_payload_target, headers=auth_headers)
    assert response.status_code == 404
    assert response.json()["detail"]["error"] == "not_found"
...
async def test_relationship_retrieval_and_deletion(client, auth_headers):
    ...
    resp_b = await client.post("/api/v1/assets", json={"type": "ip_address", "value": "1.1.1.1"}, headers=auth_headers)
    id_a = resp_a.json()["id"]
    id_b = resp_b.json()["id"]
    ...
    resp_rel = await client.post("/api/v1/relationships", json=rel_payload, headers=auth_headers)
    ...
    response = await client.delete(f"/api/v1/relationships/{rel_id}", headers=auth_headers)
